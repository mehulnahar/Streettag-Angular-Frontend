...
export class DobChangeComponent implements OnInit {
 ...
 d :any
  ...
  onSubmit(data) {
	...
	// console.log(this.d)
	var nd = new Date(this.d).toLocaleString('en-GB');
  var nnd = nd.split(',');
	console.log(nnd[0]);
    ...
    // if(data2.date_of_birth == "Invalid Date" ||  data2.date_of_birth == ""){
    //   check3 = 0;
    // }
    ...
    var url = `${this.baseUrl}updatedob`;
    ...
    this.ajaxService.post(data, url).subscribe((data) => {
      ...
      if (data["status"] == "true") {
        //console.log('ok');
        this.snackBar.open("Date Of Birth Updated Successfully.", null, {
          duration: 3000,
          verticalPosition: "top",
