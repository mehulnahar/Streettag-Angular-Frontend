...
export class VendorComponent implements OnInit {
  ...
  openEditDialog(event: any): void {
    const dialogRef = this.dialog.open(DialogEditVendor, {
      width: '600px',
      maxWidth: '95vw',
      maxHeight: '90vh',
      data: { event: event },
      panelClass: 'vendor-dialog'
    });
  ...
  getVendors() {
    ...
    this.ajaxService.get<VendorResponse>(url).subscribe((data) => {
...
export class DialogAddVendor {
  ...
  addevent(): boolean {
    if (typeof this.tag_image == "undefined") {
      this.snackBar.open("Please select before scan image", undefined, {
        duration: 2000,
        verticalPosition: "top",
      });
      return false;
...
export class DialogEditVendor {
  ...
  createForm() {
    this.angForm = this.fb.group({
      ...
      bonus_points: [this.data.event.bonus_points, [Validators.required, Validators.min(0)]],
      discount: [this.data.event.discount, [Validators.required, Validators.min(0), Validators.max(100)]],
  ...
  updateevent(): boolean {
    if (!this.tag_image) {
      this.snackBar.open("Please select an image", undefined, {
        duration: 2000,
        verticalPosition: "top",
      });
      return false;
    ...
    if (this.angForm.valid) {
      const url = `${this.baseUrl}editVendor`;
      const data = {
      ...
      this.ajaxService.post(data, url).subscribe((response) => {
...
export class DialogTagVendor {
  ...
  public tagDiameter = this.data.event.diameter || "500";
  ...
  updateevent(): boolean {
    if (typeof this.tag_image == "undefined") {
      this.snackBar.open("Please select image", undefined, {
        duration: 2000,
        verticalPosition: "top",
      });
      return false;
