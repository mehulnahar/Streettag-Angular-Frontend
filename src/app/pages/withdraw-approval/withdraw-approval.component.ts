...
export class WithdrawApprovalComponent implements OnInit {
  ...
  public dataSource: MatTableDataSource<WithdrawalData>;
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource<WithdrawalData>([]);
  }
  ...
  getWithdrawRequest() {
    ...
    const req = { approve: 0 };
    this.ajaxService.post<ApiResponse>(req, url).subscribe(
      (result) => {
        this.resData = result;
        this.dataSource = new MatTableDataSource<WithdrawalData>(result.data);
  ...
  approveRequest(id: number) {
    ...
    const req = { id: id };
    this.ajaxService.post<{ status: boolean }>(req, url).subscribe((result) => {
      this.getWithdrawRequest();
      this.snackBar.open("Request has been approved.", undefined, {
        duration: 3000,
        verticalPosition: "top",
